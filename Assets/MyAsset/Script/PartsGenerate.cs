...
public class PartsGenerate : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        // 現在のレベルに応じた生成パーツリストを取得
        lists = GameSystem.self.levelDatas[Mathf.Min(GameSystem.self.Level, GameSystem.self.levelDatas.Count - 1)].generatingParts;
    ...
    Parts Generate()
    {
        GameObject instObj = lists[Random.Range(0,lists.Count)].gameObject;
        if(instObj != null)
        {
